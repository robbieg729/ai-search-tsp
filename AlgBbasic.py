...
# set parameter values alpha, beta, rho, N, tao_zero, and max_iter (maximum number of iterations)
alpha = 1
beta = 3
rho = 0.5
N = num_cities
tao_zero = N / tour_length
max_iter = 10000
...
pheromone_levels = [[tao_zero for i in range(num_cities)] for j in range(num_cities)]
...
for i in range(0, max_iter):
    ...
    # initalize new pheromone levels matrix, which is initially a copy of the current pheromone levels matrix multiplied by 
    # (1-rho) this is so we can update the pheromone levels once each individual ant has finished its tour, without affecting the 
    # pheromone levels for the other ants (which would affect its probabilistic choices)
    new_pheromone_levels = [[(1 - rho) * p for p in pheromone_levels[j]] for j in range(num_cities)]
    ...
    for ant in ants: 
        ...
        # let the ant deposit pheromone onto its trail
        for j in range(0, num_cities):
            current_city = ant.tour[j]
            next_city = ant.tour[0] if j == num_cities - 1 else ant.tour[j + 1]
            new_pheromone_levels[current_city][next_city] += 1 / ant.tour_len
    ...
    # set the current pheromone levels to be the new ones calculated above
